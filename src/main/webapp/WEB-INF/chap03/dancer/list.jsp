...
<body>
    ...
    <%--
        c:forEach 태그를 이용하여 향상 for문을 구현하는법.
        제어변수를 선언하시고, item 속성에 반복문을 돌리고자 하는 컬렉션 자료형을 세팅
        request 객체 내에 있는 dl이라는 이름의 데이터를 el을 사용하여 꺼낼 수 있다.
        ${requestScope.dl} -> 요게 원형인데, Scope를 언급하지 않으면 dl이라는 이름의 데이터를
        전달된 모든 객체에서 뒤져서 꺼내옵니다.
    --%>
        <c:forEach var="d" items="${dl}">
            <li>
                <%-- el을 사용하여 객체를 지목한 후 메서드를 메서드를 호출할 때는 get을 제외한 나머지 이름을 얘기하면 됩니다.
                    알아서 getter를 호출해 줍니다. (필드명) --%>
    ...
    <script>
        ...
        $dancerList.onclick = e => {
            ...
            if(!e.target.matches('ul#dancer-list button')) {
                return;
            }
            ...
            // 자바스크립트로 form 없이 post요청 보내는 방법은 추후에
